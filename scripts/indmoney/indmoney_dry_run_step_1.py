...
def main() -> None:
    ...
    logger.info(
        "Starting DRY RUN step 1 | bank_files=%d | indmoney_files=%d",
        len(bank_files),
        len(indmoney_files),
    )
    ...
    # ---------------------------------------------------------------
    logger.info("=" * 70)
    logger.info("PHASE 1: Fetch deposits from bank statements")
    for file_path in bank_files:
        logger.info("[DRY RUN] Processing bank statement: %s", file_path)
        run_deposits_bank(
            pdf_path=file_path,
            dry_run=True,
        )
    ...
    logger.info("=" * 70)
    logger.info("PHASE 2: Fetch deposits from indmoney statements")
    for file_path in indmoney_files:
        logger.info("[DRY RUN] Processing indmoney statement: %s", file_path)
    ...
    # ---------------------------------------------------------------
    # Phase 3: Investment exchange rate (depends on deposits)
    # ---------------------------------------------------------------
    logger.info("=" * 70)
    ...
    # ---------------------------------------------------------------
    # Phase 4: Holdings snapshot (sorted chronologically)
    # ---------------------------------------------------------------
    logger.info("=" * 70)
    ...
    for file_path in indmoney_files_sorted:
        logger.info("[DRY RUN] Processing holdings: %s", file_path)
        run_holdings(
            pdf_path=file_path,
            dry_run=True,
        )
    ...
    logger.info(
        "DRY RUN step 1 completed | bank_files=%d | indmoney_files=%d",
        len(bank_files),
        len(indmoney_files),
    )
